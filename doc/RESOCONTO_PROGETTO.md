...
Nessun asset grafico o audio reale è presente: geometrie di test (piani, cilindri, box) e un motore audio che genera silenzio al posto di sample reali.
...
| `test:e2e` | `playwright test` | **Nessun test e2e scritto finora** — Playwright è installato ma non usato |
...
| `@playwright/test` | 1.62.0 | Framework e2e installato ma **senza test scritti**. |
...
4. Il loop (`requestAnimationFrame`) ogni frame:
   ...
   - `updateHUD()` — HP/torcia/oscurità/arma (HP e oscurità sono **ancora statici**, non collegati a `HealthStore`/un sistema di oscurità: resta gap G-03/futuro);
...
- `src/gameplay/echo/KaEchoSystem.ts` — il tasto R è mappato (`ActionKind.KaEcho`) ma `processInput` si limita a un `log.info('Ka echo')` (gap G-04).
...
- `src/simulation/Director.ts` — mai istanziato, nessuno spawn di nemici avviene.
...
- `src/progression/SaveManager.ts` e `KaProgression.ts` — mai istanziati da `GameApplication`; nessun salvataggio avviene (gap G-05).
...
- HUD con barra HP (ancora statica — gap G-03), barra torcia (dinamica, funzionante), indicatore oscurità (ancora statico), nome arma (cambia testo, nessun effetto), minimappa placeholder ("MAPPA", nessun contenuto).
- Tutorial overlay con i comandi, dismissable, attiva il pointer lock.
- Menu impostazioni (Esc) con tutti i 14 controlli di accessibilità, editabili e applicabili a `config.accessibility` — **ma nessuno di questi settaggi ha un effetto osservabile** nel renderer o nel gameplay attuale (sono salvati in memoria, non persistiti, e non letti da nessun sistema di rendering/gameplay; gap G-07).
...
`computePowerBand(purchasedNodeCount)` alimenta direttamente il budget del Director (§7.8) — stesso schema a 4 fasce.
...
- **Minimappa placeholder** (alto-destra): 140×140px, testo statico "MAPPA", nessun contenuto reale.
...
Sistema di rebind supportato a livello di dati (`ActionMap.remap()`, immutabile — restituisce una nuova istanza) ma **nessuna UI di rebind esiste** nel `SettingsMenu`.
...
## 10. Fisica (`src/physics/`) — pronta ma non collegata
...
- **`PlayerCharacterController.ts`** (399 righe, il modulo più completo e mai esercitato a runtime): `RAPIER.KinematicCharacterController` con accelerazione/decelerazione separata a terra/in aria, sprint, crouch (transizione morbida altezza capsula, ricrea il collider perché Rapier non supporta resize diretto), salto con coyote time (`PLAYER.coyoteTicks`) e input buffer (`PLAYER.inputBufferTicks`), autostep (`PLAYER.maxStepM`), snap-to-ground, terminal velocity `-30 m/s`.
...
npm run lint                →  pulito sui file toccati da G-01 (vedi nota sotto)
npm run verify:boundaries   →  nessuna violazione
npm run verify:content      →  tutti i valori nei range previsti
npx vitest run              →  15 file di test, 126 test, tutti passati
...
- Nessun test di integrazione end-to-end del loop di gioco (Playwright installato, zero test scritti).
...
**Aggiornamento (risolto durante G-01)**: `npm run typecheck` e `npm run lint` sono stati eseguiti per la prima volta durante il lavoro su G-01. Emersi e corretti: `tsconfig.json` con `baseUrl` deprecato (TS 6, richiedeva `"ignoreDeprecations": "6.0"`), 9 variabili/import inutilizzati preesistenti (`SwarmSteering.ts`, `CombatSystem.ts`, `DiggingSystem.ts`, `WeaponSystem.ts`, `Director.ts`, `PhysicsSystem.ts`, `PlayerSystem.ts`, 2 file di test), 6 errori ESLint type-checked (`no-unnecessary-condition`, `no-unnecessary-type-assertion`, `no-non-null-assertion`) nei file toccati da G-01. `npm run typecheck` sull'intero progetto ora è pulito. `npm run lint` sull'intero progetto **non è stato eseguito fino in fondo** in questa sessione perché ESLint con `strictTypeChecked`/`projectService` richiede oltre 45 secondi (limite del sandbox di sviluppo usato) — è stato verificato mirato sui soli file toccati (`npx eslint <lista file>`, che risponde in pochi secondi). Chi riprende il lavoro con più tempo a disposizione dovrebbe eseguire `npm run lint` senza restrizioni di tempo per coprire anche i file non ancora toccati da questa o precedenti sessioni.
...
## 13. Audio (`src/audio/WebAudioEngine.ts`) — funzionale ma silenzioso

Web Audio API completa: bus routing a 5 canali (`master → sfx/music/ambience/ui`, music a gain 0.6 di default), pooling con limite 32 istanze simultanee, panning HRTF 3D (`PannerNode`, modello `inverse`, `refDistance=1`, `maxDistance=30`), listener position/orientation, fade out su stop, gain ramping lineare per bus.

**Limite critico**: `generateSilence()` crea un `AudioBuffer` vuoto al posto di caricare un sample reale — non esistono asset audio nel repository (`public/` contiene solo `favicon.svg` e `icons.svg`). Il motore è pronto architetturalmente (basta sostituire `generateSilence` con un vero `decodeAudioData` da file) ma **oggi non produce mai suono udibile**, anche se venisse istanziato e collegato.

Mai istanziato da `GameApplication` — nessuna chiamata a `createAudioEngine()` nel codice applicativo.
...
> **Aggiornamento**: il gap #1 (fisica/player non collegati) è stato **chiuso** (G-01, vedi `PIANO_COLMATURA_GAP.md`). La numerazione sotto è mantenuta storicamente; il riferimento vivo e aggiornato per lo stato di ogni gap è la checklist §9 e il Registro di implementazione in cima a `PIANO_COLMATURA_GAP.md`.

1. ~~Nessun collegamento fra simulazione/fisica e il loop di gioco reale~~ — **RISOLTO (G-01)**: `PhysicsWorld` + `PlayerCharacterController` sono ora istanziati in `GameApplication.init()`, `PlayerSystem`/`PhysicsSystem` sono registrati sullo scheduler, la collisione ad-hoc `Box3` è stata rimossa da `ThreeRendererService`. Restano da collegare combattimento e IA (vedi punto 3 e G-03).
2. **`FloorGenerator` mai usato per costruire la scena reale** — collegare `GenerationClient.request()` all'avvio di `GameApplication` e costruire mesh/collider Three.js+Rapier a partire dal `FloorModel` restituito. **Prossimo gap da affrontare (G-02).**
3. **Nessun nemico esiste mai in scena** — serve un sistema di spawn che consumi `Director.canSpawn/commitSpawn`, crei entità ECS, istanzi `MummyRuntime`/`ScarabRuntime`, e li aggiorni nello scheduler (fase `ai`).
4. **Asset audio assenti** — `WebAudioEngine` produce solo silenzio; serve una libreria di sample reali (o sintesi procedurale) prima che l'audio abbia qualunque effetto percepibile.
5. **Persistenza mai collegata** — `SaveManager`/`KaProgression` esistono ma nessun salvataggio/caricamento avviene mai nel gioco.
6. **Impostazioni di accessibilità senza effetto** — 14 controlli editabili nel menu, zero collegati a un comportamento osservabile.
...
13. **`GameApplication` mantiene `playerHp`/`darknessLevel` come variabili mai riassegnate** — placeholder evidenti in attesa che il combattimento e il sistema di oscurità vengano collegati.
